...
class _MouseEvent(ABC):
    ...
    def get_mouse_button(self, key_value: int, ignore_errors: bool = None) -> [ClickType, Click]:
        ...
        for click_type in self.ClickType:
            ...
            if not ignore_errors:
                raise ValueError(f"The mouse button value '{key_value}' is not a known mouse button type")
            return self.ClickType.none, self.Click.none

    def __init_subclass__(cls, **kwargs):
        if not hasattr(cls, "Click"):
            raise NotImplementedError(f"Click(Enum) not specified in {cls.__name__}")
